...
//#define MAXSAMPLES 200 // number of messages to print in here above test
...
int iter = 0;
extern const unsigned char flip[]; // content declared at the end of this file
struct __attribute__((packed)) {
    uint8_t  flags;
    uint32_t towMillis;
    uint16_t year;
...
inline void txArincFrame() {
  ...
  bool vld=true;
  WriteArincWord(BuildArincWord(hvelRange, 10*knot, vld?3:1, 0b11, flip[0104]));
  WriteArincWord(BuildArincWord(hvelRange, -7*knot, vld?3:1, 0b11, flip[0106]));
  #define ftPerMin (0.3048/60*1000.) //ft/min in mm/s
  #define vvelRange (32768.*ftPerMin)
  WriteArincWord(BuildArincWord(vvelRange, +3*ftPerMin, vld?3:1, 0b11, flip[0107]));

  // latitude, longitude, height
  #define deg (2147483648./180.) // degree in cordic
  #define ft 0.3048
  #define heiRange (131072.*ft)
  double lat=44.0, lon=3.6, hei=300*ft;
  int32_t iLat = round(lat*deg), iLon = round(lon*deg);
  ...
  WriteArincWord(BuildArincWord(heiRange,       hei, vld?3:1, NOSDI, flip[0370]));
  WriteArincWord(BuildArincWord(2048., iLat & 0x7ff, vld?3:1,  0b11, flip[0120]));
  WriteArincWord(BuildArincWord(2048., iLon & 0x7ff, vld?3:1,  0b11, flip[0121]));

  // UTC
  uint8_t hour=15, minute=33, second=04;
  // total of 18 bits:                             5        6         6         1
  WriteArincWord(BuildArincWord(262144., (uint32_t)hour<<13|minute<<7|second<<1|1, vld?3:1, 0b11, flip[0150])); // hours, minutes, seconds
  uint32_t nanos=724362002; //0.724 seconds = 777777777*2^-30 seconds
  uint64_t secFrac = ((uint64_t)nanos<<30)/1000000000;  // nanoseconds > 2^-30 seconds
  WriteArincWord(BuildArincWord(1048576.,  secFrac>>10, vld?3:1, NOSDI, flip[0140])); // second fraction coarse
  WriteArincWord(BuildArincWord(1024., secFrac & 0x3ff, vld?3:1,  0b11, flip[0141])); // second fraction fine
  uint8_t day=30, month=9, year=57;
  uint8_t tday=day/10, uday=day%10, tmon=month/10, umon=month%10, tyea=year/10, uyea=year%10;
  // total of 19 bits:                             2        4        1        4       4       4
  WriteArincWord(BuildArincWord(262144., (uint32_t)tday<<17|uday<<13|tmon<<12|umon<<8|tyea<<4|uyea, vld?0:1, 0b11, flip[0260])); // day, month, year

  // DOPS
  uint16_t hdop=5555, vdop=6666; // 55.55, 66.66
  WriteArincWord(BuildArincWord(102400., hdop, vld?3:1, 0b11, flip[0101])); // HDOP
  WriteArincWord(BuildArincWord(102400., vdop, vld?3:1, 0b11, flip[0102])); // VDOP
...
//"à tester"
  ...
  WriteArincWord(BuildArincWord(32768., velNedCov.svCount << 10 | velNedCov.statusFlags & 0x1f << 5 | llhPosCov.statusFlags & 0x1f, vld?0:1, 0b11, flip[0273]));
  
  // System monitor
  if (devMonRefreshed) {
    int16_t cpuTemp=3456, feTemp=6543; // should display 35°C and 65°C (rounded to nearest integer Celsius degree)
    WriteArincWord(BuildArincWord(131072., (int32_t)(0.01* feTemp+0.5)<<9|(int32_t)(0.01*cpuTemp+0.5)&0x1ff, vld?0:1, 0b11, flip[0357]));
    devMonRefreshed = false;
  }
}

// delayMicoseconds() with 32-bit integer argument
inline extern void myDelayUS(uint32_t d) {
